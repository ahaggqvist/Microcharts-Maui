    ...
    public abstract class AxisBasedChart : SeriesChart
    {
        ...
        public override void DrawContent(SKCanvas canvas, int width, int height)
        {
            if (Series != null && entries != null)
            {
                ...
                var fixedRange = InternalMaxValue.HasValue || InternalMinValue.HasValue;
                ...
                width = MeasureHelper.CalculateYAxis(ShowYAxisText, ShowYAxisLines, entries, YAxisMaxTicks, YAxisTextPaint, YAxisPosition, width, fixedRange, ref maxValue,
                    ref minValue, out var yAxisXShift, out var yAxisIntervalLabels);
                var valRange = maxValue - minValue;
                ...
                DrawHelper.DrawYAxis(ShowYAxisText, ShowYAxisLines, YAxisPosition, YAxisTextPaint, YAxisLinesPaint, Margin, AnimationProgress, maxValue, valRange, canvas, width,
                    yAxisXShift, yAxisIntervalLabels, headerHeight, itemSize, origin);

                var nbSeries = series.Count();
                for (var serieIndex = 0; serieIndex < nbSeries; serieIndex++)
                {
                    ...
                    for (var i = 0; i < labels.Length; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(label))
                        {
                            ...
                            DrawHelper.DrawLabel(canvas, LabelOrientation, YPositionBehavior.None, itemSize, new SKPoint(itemX, height - footerWithLegendHeight + Margin),
                                LabelColor, labelSize, label, LabelTextSize, Typeface);
                        }
                    }
                    ...
                    for (var i = 0; i < labels.Length; i++)
                    {
                        ...
                        if (entry != null && entry.Value.HasValue)
                        {
                            ...
                            var marge = serieIndex < nbSeries ? Margin / 2 : 0;
                            var totalBarMarge = serieIndex * Margin / 2;
                            var barX = itemX + serieIndex * barSize.Width + totalBarMarge;
                            var barY = headerWithLegendHeight + ((1 - AnimationProgress) * (origin - headerWithLegendHeight) +
                                                                 (maxValue - value) / valRange * itemSize.Height * AnimationProgress);
        ...
        protected virtual void DrawValueLabel(SKCanvas canvas, Dictionary<ChartEntry, SKRect> valueLabelSizes, float headerWithLegendHeight, SKSize itemSize, SKSize barSize,
            ChartEntry entry, float barX, float barY, float itemX, float origin)
        {
            var label = entry?.ValueLabel;
            if (!string.IsNullOrEmpty(label))
            {
                DrawHelper.DrawLabel(canvas, ValueLabelOrientation, YPositionBehavior.UpToElementHeight, barSize,
                    new SKPoint(barX - itemSize.Width / 2 + barSize.Width / 2, headerWithLegendHeight - Margin), entry.ValueLabelColor.WithAlpha((byte)(255 * AnimationProgress)),
        ...
        private void DrawLegend(SKCanvas canvas, SKRect[] seriesNameSize, float legendHeight, float height, float width)
        {
            ...
            var lineHeight = Math.Max(seriesNameSize.Where(b => !b.IsEmpty).Select(b => b.Height).FirstOrDefault(), SerieLabelTextSize);
            ...
            for (var i = 0; i < series.Length; i++)
            {
                ...
                var legentItemWidth = Margin + SerieLabelTextSize + Margin + serieBound.Width;
        ...
        private float GenerateSerieLegend(SKCanvas canvas, float lineHeight, float origin, int nbLine, float currentWidthUsed, ChartSerie serie)
        {
            ...
            using (var paint = new SKPaint())
            {
                ...
                var textYPosition = rect.Bottom - (rect.Bottom - rect.Top) / 2 + bounds.Height / 2;
        ...
        private float CalculateLegendSize(SKRect[] seriesSizes, float serieLabelTextSize, int width)
        {
            ...
            {
                var legentItemWidth = Margin + serieLabelTextSize + Margin + rect.Width;
                if (legentItemWidth > width)
                {
                    if (currentWidthUsed != 0)
                    {
            ...
            var height = Math.Max(seriesSizes.Where(b => !b.IsEmpty).Select(b => b.Height).FirstOrDefault(), serieLabelTextSize);
        ...
        protected abstract void DrawBar(ChartSerie serie, SKCanvas canvas, float headerHeight, float itemX, SKSize itemSize, SKSize barSize, float origin, float barX, float barY,
            SKColor color);
