...
namespace Microcharts
{
    ...
    public class PointChart : AxisBasedChart
    {
        ...
        /// <inheritdoc />
        protected override void DrawValueLabel(SKCanvas canvas, Dictionary<ChartEntry, SKRect> valueLabelSizes, float headerWithLegendHeight, SKSize itemSize, SKSize barSize,
            ChartEntry entry, float barX, float barY, float itemX, float origin)
        {
            var label = entry?.ValueLabel;
            ...
            {
                DrawHelper.DrawLabel(canvas, ValueLabelOrientation, ValueLabelOrientation == Orientation.Vertical ? YPositionBehavior.UpToElementHeight : YPositionBehavior.None,
                    barSize, new SKPoint(drawedPoint.X, drawedPoint.Y - PointSize / 2 - Margin / 2), entry.ValueLabelColor.WithAlpha((byte)(255 * AnimationProgress)),
                    valueLabelSizes[entry], label, ValueLabelTextSize, Typeface);
            }
            else if (ValueLabelOption == ValueLabelOption.OverElement)
            {
                DrawHelper.DrawLabel(canvas, ValueLabelOrientation,
                    ValueLabelOrientation == Orientation.Vertical ? YPositionBehavior.UpToElementMiddle : YPositionBehavior.DownToElementMiddle, barSize,
                    new SKPoint(drawedPoint.X, drawedPoint.Y), entry.ValueLabelColor.WithAlpha((byte)(255 * AnimationProgress)), valueLabelSizes[entry], label, ValueLabelTextSize,
                    Typeface);
            }
        }

        /// <inheritdoc />
        protected override void DrawBar(ChartSerie serie, SKCanvas canvas, float headerHeight, float itemX, SKSize itemSize, SKSize barSize, float origin, float barX, float barY,
            SKColor color)
        {
        ...
        protected override void DrawBarArea(SKCanvas canvas, float headerHeight, SKSize itemSize, SKSize barSize, SKColor color, float origin, float value, float barX, float barY)
        {
            if (PointAreaAlpha > 0)
            {
                ...
                using (var shader = SKShader.CreateLinearGradient(new SKPoint(0, origin), new SKPoint(0, barY),
                           new[] { color.WithAlpha(PointAreaAlpha), color.WithAlpha((byte)(PointAreaAlpha / 3)) }, null, SKShaderTileMode.Clamp))
