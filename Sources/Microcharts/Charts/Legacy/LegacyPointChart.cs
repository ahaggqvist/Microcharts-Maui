    ...
    public class LegacyPointChart : SimpleChart
    {
        ...
        public override void DrawContent(SKCanvas canvas, int width, int height)
        {
            if (Entries != null)
            {
                ...
                width = MeasureHelper.CalculateYAxis(ShowYAxisText, ShowYAxisLines, Entries, YAxisMaxTicks, YAxisTextPaint, YAxisPosition, width, false, ref maxValue, ref minValue,
                    out var yAxisXShift, out var yAxisIntervalLabels);
                ...
                DrawHelper.DrawYAxis(ShowYAxisText, ShowYAxisLines, YAxisPosition, YAxisTextPaint, YAxisLinesPaint, Margin, AnimationProgress, MaxValue, ValueRange, canvas, width,
                    yAxisXShift, yAxisIntervalLabels, headerHeight, itemSize, origin);
        ...
        protected virtual void DrawAreas(SKCanvas canvas, SKPoint[] points, SKSize itemSize, float origin, float headerHeight)
        {
            if (points.Length > 0 && PointAreaAlpha > 0)
            {
                for (var i = 0; i < points.Length; i++)
                {
                    ...
                    using (var shader = SKShader.CreateLinearGradient(new SKPoint(0, origin), new SKPoint(0, point.Y),
                               new[] { entry.Color.WithAlpha(PointAreaAlpha), entry.Color.WithAlpha((byte)(PointAreaAlpha / 3)) }, null, SKShaderTileMode.Clamp))
        ...
        protected virtual void DrawLabels(SKCanvas canvas, string[] texts, SKPoint[] points, SKRect[] sizes, SKColor[] colors, Orientation orientation, bool isTop, SKSize itemSize,
            float height)
        {
            if (points.Length > 0)
            {
                for (var i = 0; i < points.Length; i++)
                {
                    ...
                    {
                        DrawHelper.DrawLabel(canvas, orientation, isTop ? YPositionBehavior.UpToElementHeight : YPositionBehavior.None, itemSize, points[i], colors[i], sizes[i],
                            texts[i], LabelTextSize, Typeface);
                    }
                }
            }
        }
        ...
        {
            return MeasureLabels(new[] { label }, paint).First();
        }
