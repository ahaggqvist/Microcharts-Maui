...
namespace Microcharts
{
    ...
    public class BarChart : AxisBasedChart
    {
            ...
            ChartEntry entry, float barX, float barY, float itemX, float origin)
        {
            ...
            var (location, size) = GetBarDrawingProperties(headerWithLegendHeight, itemSize, barSize, 0, barX, barY);
            ...
            else if (ValueLabelOption == ValueLabelOption.TopOfElement)
            {
                DrawHelper.DrawLabel(canvas, ValueLabelOrientation, ValueLabelOrientation == Orientation.Vertical ? YPositionBehavior.UpToElementHeight : YPositionBehavior.None,
                    barSize, new SKPoint(location.X + size.Width / 2, barY - Margin), entry.ValueLabelColor.WithAlpha((byte)(255 * AnimationProgress)), valueLabelSizes[entry],
            ...
            else if (ValueLabelOption == ValueLabelOption.OverElement)
            {
                DrawHelper.DrawLabel(canvas, ValueLabelOrientation,
                    ...
                    new SKPoint(location.X + size.Width / 2, barY + (origin - barY) / 2), entry.ValueLabelColor.WithAlpha((byte)(255 * AnimationProgress)), valueLabelSizes[entry],
                    entry.ValueLabel, ValueLabelTextSize, Typeface);
            }
        }
            ...
            SKColor color)
        {
            ...
            {
                var (location, size) = GetBarDrawingProperties(headerHeight, itemSize, barSize, origin, barX, barY);
        ...
        protected override void DrawBarArea(SKCanvas canvas, float headerHeight, SKSize itemSize, SKSize barSize, SKColor color, float origin, float value, float barX, float barY)
        {
            if (BarAreaAlpha > 0)
            {
                using (var paint = new SKPaint
                       {
                           Style = SKPaintStyle.Fill,
                           Color = color.WithAlpha((byte)(BarAreaAlpha * AnimationProgress))
                       })
                {
